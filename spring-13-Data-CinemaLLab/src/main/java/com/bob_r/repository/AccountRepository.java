...
import com.bob_r.entity.Account;
import com.bob_r.entity.Location;
...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> findByAgeIsLessThan(Integer age);
    ...
    List<Account> findAllByAddressStartingWith (String nameStart);
   ...
   List<Account> fetchAllByUsingJPQL();
    ...
    List<Account>  fetchAAdminAccount();

    //Write a JPQL query to sort all accounts with age
    @Query ("select a from Account a order by a.age asc ")
    List<Account>  fetchAllOrderBaseOrderAge();


    // ------------------- Native QUERIES ------------------- //

    //Write a native query to read all accounts with an age lower than a specific value
    @Query(value = "SELECT * FROM  account_details where account_details.age <?1",nativeQuery = true)
    List <Account> fetchAllAccountAgeLowerThanSpecificValue(@Param("age") Integer age);
    //Write a native query to read all accounts that a specific value can be containable in the name, address, country, state city
    //> concat('%",?1,'%'  where name ILIKE '%'||?1||'%'"
